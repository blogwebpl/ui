...
import {
  require_prop_types
} from "./chunk-NJSPZWO6.js";
import "./chunk-MENXWWNV.js";
import "./chunk-PMN6KPNP.js";
import {
...
var getReactSymbolName = (elementType) => elementType.toString().replace(/^Symbol\((.*)\)$/, "$1").split(".").map((segment) => segment.split("_").map(toPascalCase).join("")).join(".");
...
var renderJsx = (code, options) => {
  ...
  typeof (options == null ? void 0 : options.displayName) == "string" ? displayNameDefaults = { showFunctions: true, displayName: () => options.displayName } : displayNameDefaults = { displayName: (el) => el.type.displayName || typeof el.type == "symbol" ? getReactSymbolName(el.type) : getDocgenSection(el.type, "displayName") || (el.type.name !== "_default" ? el.type.name : null) || (typeof el.type == "function" ? "No Display Name" : null) || (isForwardRef2(el.type) ? el.type.render.name : null) || (isMemo2(el.type) ? el.type.type.name : null) || el.type };
